...
struct RadosTestContext
{
    ...
    max_in_flight(max_in_flight)
  {
    ...
    rados.init(0);
    rados.ioctx_open(pool_name.c_str(), pool);
...
struct WriteOp : public TestOp
{
  ...
  void begin()
  {
    ...
    context.pool.aio_write(context.prefix+oid,
			   completion,
			   write_buffer,
			   0,
			   to_write.str().length());
...
struct ReadOp : public TestOp
{
  ...
  void begin()
  {
    ...
    context.unlock();
    context.pool.aio_read(context.prefix+oid,
			  completion,
			  &result,
			  0,
			  old_value.length());
...
struct SnapCreateOp : public TestOp
{
  ...
  void begin()
  {
    ...
    context.pool.snap_create(snap_name.str().c_str());
  }
};
...
struct SnapRemoveOp : public TestOp
{
  ...
  void begin()
  {
    ...
    context.pool.snap_remove(snap_name.str().c_str());
  }
};
...
struct RollbackOp : public TestOp
{
  ...
  void begin()
  {
    ...
    context.pool.rollback(context.prefix+oid,
			  snap_name.str().c_str());
  }
};
